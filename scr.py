...
def add_login(persone, ignore_list):
    if persone['phone_number'] != '' and persone['name'] != '' and not any(
                    person['name'].lower() == i.lower() for i in ignore_list):
...
if data["stderr"] == '':
    ...
    write_to_file(keepass_file, get_str_for_keepass(data, title))
    write_to_file(txt_file, get_str_for_txt(data))
else:
    print (data["stderr"])
